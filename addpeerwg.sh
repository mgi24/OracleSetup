...
# Persist ke file config TANPA merapikan ulang (agar komentar tetap aman)
PSK_LINE_FILE=""
if [[ "$USE_PSK_FLAG" == true ]]; then
  PSK_LINE_FILE="PresharedKey = $PRESHARED"
fi
...
if grep -qF "$PUBKEY" "$WG_CONF" 2>/dev/null; then
...
else
  {
    ...
    [[ -n "$PSK_LINE_FILE" ]] && echo "$PSK_LINE_FILE"
    echo "AllowedIPs = 10.8.0.$PEER_IP/32"
  } >> "$WG_CONF"
fi

# Sinkronkan runtime dari file tanpa down interface
